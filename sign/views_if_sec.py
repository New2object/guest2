...
----先进行AES加密,然后把AES加密的字符串再次通过base64加密,因为通常AES加密后的字符串跟URL的格式差不多,长度也过长,不利于传输,
----所以才要进一步通过base64模块下的urlsafe_b64encode()方法对AES加密串进行二次加密
...
def get_event_list(request):
    ...
    if auth_result == "success":
        if request.method == "GET":
            ...
            return JsonResponse({'status': 10015, 'message': 'Request Method Not GET'})
...
# ===================处理客户端请求的时间========================== #
    ...
    # 签名检查
    md5 = hashlib.md5()
    sign_str = client_time + "&Guest-Bugmaster"        # 时间戳+安全密钥
...
def aes_encryption(request):
    ...
    if request.method == 'POST':
        # 判断客户端请求是否为post，通过post.get()方法接收data参数
        data = request.POST.get("data", "")

        # 解密  调用解密函数decryptAES(),传参加密字符串和app_key
...
def user_auth(request):
    ...
    auth = get_http_auth.split()
    print(auth)
    # print(base64.b64decode(auth[1]).decode('utf-8'))
    try:
...
def user_sign(request):
    ...
    # 签名检查
    md5 = hashlib.md5()
    sign_str = client_time + "&Guest-Bugmaster"    # &Guest-Bugmaster-->>密钥字符串
    sign_bytes_utf8 = sign_str.encode(encoding="utf-8")
    md5.updete(sign_bytes_utf8)
    sever_sign = md5.hexdigest()
    if sever_sign != client_sign:
        return "sign error"
    else:
