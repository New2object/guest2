...
# 点击对话框中的链接
# 由于对话框中的Element被蒙板遮挡,直接点击会报 Element is not clickable的错误
# So 使用JS来模拟click
# 在watir-webdriver中只需要fire-event(:click)就可以了
link = dr.find_element_by_id('myModal').find_element_by_id('click')
dr.execute_script('$(arguments[0]).click()', link)
sleep(2)

# 关闭对话框
buttons = dr.find_element_by_class_name('modal-footer').find_element_by_tag_name('button')
buttons[0].click()
